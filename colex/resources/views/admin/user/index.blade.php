            ...
            <tbody>
                @foreach ($users as $user)
                    <tr>
                        ...
                        <td class="align-middle">{{ $user->roles[0]->display_name ?? '' }}</td>
                        <td class="align-middle">
                            @can('show-users')
                                <a href="{{ url("admin/users/$user->id") }}" class="btn btn-info btn-icon btn-sm" title="Detail">
                                    <i class="fas fa-info-circle"></i>
                                </a>
                            @endcan
                            @can('edit-users')
                                <a href="{{ url("admin/users/$user->id/edit") }}" class="btn btn-success btn-icon btn-sm" title="Edit">
